...
int wmain(int argc, wchar_t* argv[])
{
	...
	try
	{
		max_length_int = _wtoi(argv[2]);
		min_length_int = _wtoi(argv[3]);
		num_beams_int = _wtoi(argv[4]);
		num_return_sequences_int = _wtoi(argv[5]);
		length_penalty_float = _wtof(argv[6]);
		repetition_penalty_float = _wtof(argv[7]);
	}
	catch (const std::exception& e)
	{
